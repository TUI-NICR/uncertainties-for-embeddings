...
def distribution_contours(x_values, y_values, x_min=None, x_max=None,
                          y_min=None, y_max=None,
                          levels=[0.004, 0.01, 0.03, 0.1, 0.3, 0.7, 0.9],
                          linewidths=0.5, colors='k', bandwidth=0.65):
    ...
    x_fac = 10 / diff_x
    y_fac = 7.5 / diff_y
    xx, yy, zz = kde2D(x_values*x_fac, y_values*y_fac, x_min*x_fac - 1, x_max*x_fac + 1,
                       y_min*y_fac - 1, y_max*y_fac + 1, bandwidth=bandwidth)
    xx /= x_fac
    yy /= y_fac
...
def distribution_plot(x_values, y_values, x_min=None, x_max=None, y_min=None,
                      ...
                      filename=None, save_instead_of_show=True, y_bbox=no_bbox, x_bbox=no_bbox, fontsize=20, trendline_style="b-"):
    ...
    # x_fac = 10 / diff_x
    # y_fac = 7.5 / diff_y
    # xx, yy, zz = kde2D(x_values*x_fac, y_values*y_fac, x_min*x_fac, x_max*x_fac,
    #                    y_min*y_fac, y_max*y_fac, bandwidth=0.65)
    # xx /= x_fac
    # yy /= y_fac
    # plt.contour(xx, yy, zz, levels=[0.0015, 0.005, 0.02, 0.05, 0.1, 0.125, 0.15],
    #             linewidths=0.5, colors='k')

    distribution_contours(x_values, y_values)
    
    #print("polyfit")
    z = np.polyfit(x_values, y_values, 1)
    p = np.poly1d(z)
    x_line = np.linspace(x_min, x_max, 2)
    plt.plot(x_line, p(x_line), trendline_style)
    # plt.text(0.5, 0.5, r"asdasd")
    plt.text(
        x=0.95, y=0.95,           # Position of the text (normalized to the axes)
        s=f'correlation $= {np.corrcoef([np.abs(x_values), y_values])[0, 1]:.4f}$',      
        transform=plt.gca().transAxes,  # Use the Axes coordinates
        ha='right',                     # Horizontal alignment to the right
        va='top',                       # Vertical alignment to the top
        fontsize=12,                    # Font size
        color='black',                  # Text color
        bbox=dict(facecolor='white', alpha=0.5, edgecolor='none')  # Background with transparency
    )
    if save_instead_of_show:
        plt.tight_layout()
...
def get_lambda_weighting_performances(all_data, augmentation_basis, augmentation_basis_is_std, maximization_goal, augmentation_auxiliary, augmentation_auxiliary_is_std, base_mAPs, base_rank1s, norm_func=lambda x: torch.norm(x, 2, 1, keepdim=True), hint_value = 0.2438, additional_lambda_values=[]):
    """calculate mAP and rank1 for all given runs when weighing the feature vector with 1/(uncertainty + eps)
    ...
    for data in tqdm(all_data, "Hyperparameter Search"):
        ...
        diff = delta * (factor ** np.arange(10, iterations+1) - sub)
        lambda_values = sorted([0, hint_value] + list(hint_value - diff) +
                                list(hint_value + diff) + additional_lambda_values)
...
def get_all_performances(paths, augmentation_bases, out_path, augmentation_auxiliaries=["model", "data", "dist"]):
    ...
    for augmentation_basis in augmentation_bases: 
        ...
        for augmentation_basis_is_std in [True, False]:
            ...
            for maximization_goal in ["best_avg_delta-mAP"]:#, "best_avg_res-mAP"]: # does not make a difference
                ...
                for augmentation_auxiliary in augmentation_auxiliaries:
                    ensure_key_exists(
                    ...
                    for augmentation_auxiliary_is_std in [True, False]:
                        ...
                        lambda_weighting_mAPs, lambda_weighting_rank1s = get_lambda_weighting_performances(
                            ...
                            [1024, 3333, 1010, 1066, 1111, 1069, 1337, 2222, 1536, 666, 4444, 5555, 6666, 7777, 8888, 9999] )
                        ...
                        lambda_weighting_mAPs, lambda_weighting_rank1s = get_lambda_weighting_performances(
                            ...
                            [1, 11, 22, 3, 6, 9, 33, 99, 666, 69, 420] )
...
#tabulate_all_results_summary(path="UAL_results_model.json", precision=5, exclude_keys=["best_avg_res-mAP"])
...
"""get_all_performances( 
  ...
  # bsub -J "MoDi" -q "highmem" -outdir . -o ./lout.%J -e ./lerr.%J python ../../misc/data_stats.py 
# -R "select[hname!='makalu94']"
# -R "select[hname!=makalu158 && hname!=makalu145]"
# -w "done(2643946)"


"""data = load_data()
uncertainty_type = "variance_of_mean_vector"
plot_exemplary_correlation(data, uncertainty_type,
                               qi=1003, gi1=10000, gi2=7146,
                               save_instead_of_show=True)"""
...
def create_distractor_set_json_file():
    data = load_data()

    ret = {}
    for set_id in ["D1", "D2", "D3", "D4"]:
        ret[set_id] = data["sets"][set_id]
        print(set_id, len(ret[set_id]))

    with open("distractor_sets.json", "w") as f:
        json.dump(ret, f)

create_distractor_set_json_file()