...
NUM_STDS_FOR_CUTOFF_LARGE = 3

DATA_UNC_MEAN_NORM = 2690.320824614789
DATA_UNC_NORM_STD = 9.705868017852563
DIST_UNC_MEAN_NORM = -6344.823391268496
DIST_UNC_NORM_STD = 183.6124799374791
...
DIST_CUTOFF_STR = r"{}".format(f"{DIST_CUTOFF:.0f}")
DATA_CUTOFF_STR = r"{}".format(f"{DATA_CUTOFF:.0f}")
DIST_CUTOFF_LARGE_STR = r"{}".format(f"{DIST_CUTOFF_LARGE:.0f}")
...
def get_vectors(data, set_id, vector_type):
    ...
    if vector_type == "fDATA": # filtered data uncertainty vectors whose images do not also have high distributional uncertainty
        return [np.array(data['data'][name][VAR_VEC])
                ...
                if 1024 * np.log(2 * np.pi * np.e) + 0.5 * np.sum(np.log(np.array(data['data'][name][VAR_OF_VAR]))) < DIST_CUTOFF]
        ...
        return [np.array(data['data'][name][VAR_OF_VAR])
                ...
                if 1024 * np.log(2 * np.pi * np.e) + 0.5 * np.sum(np.log(np.array(data['data'][name][VAR_VEC]))) < DATA_CUTOFF]
        ...
        return [np.array(data['data'][name][VAR_VEC])
                ...
                if 1024 * np.log(2 * np.pi * np.e) + 0.5 * np.sum(np.log(np.array(data['data'][name][VAR_OF_VAR]))) > DIST_CUTOFF_LARGE]
...
def generate_uncertainty_score_distribution_plots():
    ...
    for uncertainty_type in [VAR_VEC, VAR_OF_VAR, VAR_OF_MEAN, "fDATA", "fDIST", "fDATA+"]:
        ...
        for set_id in ["Q", "D1", "D2", "D3", "D4"]:
            ...
            # print(f"{uncertainty_type=}, {np.max(vectors)=}, {np.min(vectors)=}, {np.sum(np.array(vectors) >= 1)} = {100*np.sum(np.array(vectors) >= 1)/np.sum(np.array(vectors) >= float('-inf')):.5f}% are >=1")
            # sum all >=1 values (their ln) und all <1 values (their ln)
            # print(f"{np.sum(np.log(np.array(vectors)[np.array(vectors) >= 1]))} vs. {np.sum(np.log(np.array(vectors)[np.array(vectors) < 1]))}")
            # print(f"--- {100*np.min(np.sum(np.where(np.array(vectors) >= 1, np.log(np.array(vectors)), 0), axis=1) / np.sum(np.where(np.array(vectors) < 1, np.log(np.array(vectors)), 0), axis=1)):.5f}% of the good values are bad values")
            ...
            norms = 1024 * np.log(2 * np.pi * np.e) + 0.5 * np.sum(np.log(vectors), axis=1) # entropy
        ...
        for set_id in ["Q", "D1", "D2", "D3", "D4"]:
            ...
            if uncertainty_type in ["fDATA", "fDIST", "fDATA+"]:
                bandwidth=(norm_range_size*0.06)
            ...
            #print(f"{uncertainty_type=}, {set_id=}, mu = {mean_norm}, std = {norm_std}")
                    
            if uncertainty_type == "fDATA" and set_id == "D4":
                print(f"{norms=}")
                plt.plot([norms[0], norms[0]], [0, 0.00618], "-", color=set_color, linewidth=2, label=set_id)
                plt.plot([norms[1], norms[1]], [0, 0.00618], "-", color=set_color, linewidth=2)
                # TODO: copy sig scale code from below once updated
                #continue # 2 samples (@ 1.5 sig) is not enough for meaningful KDE
            else:
                if max_y > max_max_y:
                    max_max_y = max_y
            ...
            if set_id == "Q":
                ...
                i = 0
                left_extend = 1
                if uncertainty_type == VAR_OF_MEAN:
                    left_extend = 7
                ...
                plt.text(mean_norm - (left_extend+0.15)*norm_std, std_axis_y, "means:  ", horizontalalignment="right", verticalalignment="center", fontsize=11)

                plt.plot([mean_norm - left_extend*norm_std, mean_norm + i*norm_std], [std_axis_y, std_axis_y], "-", color=set_color, linewidth=0.5)
            else:
                print(f"{set_id=}, mu = mu_Q + {(mean_norm - mean_Q)/std_Q:.2f} * std_Q, std = {norm_std / std_Q:.2f} * std_Q")
                #print(f"{set_id=}, mu = mu_Q + {(mean_norm - mean_Q)/norm_std:.2f} * std")
                offset = {
                    "D1": 1,
                    "D2": 2,
                    "D3": 3,
        ...
        x_label = {
            VAR_VEC: r"Data Uncertainty Score $\mathcal{H}( \Sigma^{(D)} )$",
            VAR_OF_VAR: r"Distributional Uncertainty Score $\mathcal{H}( \Sigma^{(V)} )$",
            VAR_OF_MEAN: r"Model Uncertainty Score $\mathcal{H}( \Sigma^{(M)} )$",
            "fDATA": r'Data Uncertainty Score $\mathcal{H}( \Sigma^{(D)} )$',
            "fDATA+": r'Data Uncertainty Score $\mathcal{H}( \Sigma^{(D)} )$',
            "fDIST": r"Distributional Uncertainty Score $\mathcal{H}( \Sigma^{(V)} )$"
        }[uncertainty_type]
        x_label_bbox = {
            VAR_VEC: bbox_dat,
            VAR_OF_VAR: bbox_dis,
        ...
        if uncertainty_type == "fDATA":
            plt.title(r"Considering images where $\mathcal{H}( \Sigma^{(V)} ) <" + DIST_CUTOFF_STR + r"$", fontsize=15)
        elif uncertainty_type == "fDIST":
            plt.title(r"Considering images where $\mathcal{H}( \Sigma^{(D)} ) <" + DATA_CUTOFF_STR + r"$", fontsize=15)
        elif uncertainty_type == "fDATA+":
            plt.title(r"Considering images where $\mathcal{H}( \Sigma^{(V)} ) >" + DIST_CUTOFF_LARGE_STR + r"$", fontsize=15)

        plt.axis([x_axis_min, x_axis_max, 0, max_max_y * 1.22])
...
def tabulate_all_results_summary(path="all_UAL_results.json", precision=4, show_mAP=True, exclude_keys=[], exclude_paths_with_substr=[], path_sep="/", paths=[], whitelist_paths_with_substr=[]):
    ...
    if len(paths) == 0:
        paths = [path]
    for path in paths:
        result = read_all_results(path=path)
        results[path] = result
    ...
    def collect_paths_and_values(dic, path="") -> list[list]:
        ...
        for k,v in dic.items():
            ...
            if "all" in v.keys():
                should_skip_wl = True
                # print(new_path)
                for wl_str in whitelist_paths_with_substr:
                    if wl_str in new_path:
                        should_skip_wl = False
                if (should_skip_wl and len(whitelist_paths_with_substr) > 0):
                    continue
                if v["mean"] > 0.871 or True:
                    # found a leaf
                    rv.append([
                    ...
                    if "eps" in v.keys():
                        rv[-1].append(v["eps"])
                    if "best_lambda" in v.keys():
                        rv[-1].append(v["best_lambda"])
                    if "model/False/fv_weighting/best_avg_delta-mAP/lambda/dist/False/1_l1" in new_path:
                        #print(get_stats_container(best_lambdas(v["lambda_mAP_map"])), new_path)
                        print(v["all"][0], "***", v["max"], new_path)
                    if "model/False/fv_weighting_nomagic" in new_path:
                        print(v["all"][0], "***", v["max"], new_path)
                    if "model/False/fv_weighting/best_avg_delta-mAP/eps" in new_path:
                        print(v["all"][0], "***", v["max"], new_path)
    ...
    for path in paths:
        mAP_summary, best_mAP = collect_paths_and_values(results[path]["mAPs"], path=path[11:-5])
        best_mAPs[path] = best_mAP
        mAP_summaries += mAP_summary

        rank1_summary, best_rank1 = collect_paths_and_values(results[path]["rank1s"], path=path[11:-5]) 
        rank1_summaries += rank1_summary
        best_rank1s[path] = best_rank1

    # _, best_mAPs = collect_paths_and_values(best_mAPs)
    ...
    if show_mAP:
        # sort by mean mAP over runs
        # print(mAP_summaries[0])
        # print(mAP_summaries[0][0])
        # print(mAP_summaries[0][0][2+precision+2:4+precision+2+precision])
        # exit()
        def is_unique(item, seen):
            # print(item)
            # Add your custom logic here to determine if the item is a duplicate
            # For this example, I'll assume case-insensitive comparison

            if "vanilla" not in item[2]:
                path = item[2].split("/aug")[1]
            else:
                path = "vanilla"
            #val = item[0][2+precision+2:4+precision+2+precision] # sorting by mean mAP
            ...
            seen.append(item_lower)
            return True
        
        sorted_indices = sorted(range(len(mAP_summaries)), 
                        key=lambda i: mAP_summaries[i][0][2+precision+2:4+precision+2+precision], 
                        reverse=True)
        sorted_mAP_summaries = [mAP_summaries[i] for i in sorted_indices]
        sorted_rank1_summaries = [rank1_summaries[i] for i in sorted_indices]
        # rows = sorted(mAP_summaries, key=lambda x: x[0][2+precision+2:4+precision+2+precision], reverse=True)
        rows = sorted_mAP_summaries
        seen = []
        rows = [row for row in rows if is_unique(row, seen)]# filter just keeps the best of same name because sorted before
    ...
    for i, row in enumerate(rows):
        ...
        nprec = precision-2
        mAP_tex = f"{float(row[0][2+precision+2:4+precision+2+precision])*100:.{nprec}f}"
        mAP_std_tex = f"{float(row[0][2+precision+2+2+precision+3:4+precision+2+precision+2+precision+3])*100:.{nprec}f}"
        r1_tex = f"{float(sorted_rank1_summaries[i][0][2+precision+2:4+precision+2+precision])*100:.{nprec}f}"
        r1_std_tex = f"{float(sorted_rank1_summaries[i][0][2+precision+2+2+precision+3:4+precision+2+precision+2+precision+3])*100:.{nprec}f}"
        # vanilla
        if split_path[1] == "vanilla":
            name = r"D^{\mu}"
        # additive
        else:
            basis_letter = {
            ...
            if split_path[3] == "True":
                aug_basis = r"\sqrt{\Sigma^{("+ basis_letter +r")}}"
            else:
                aug_basis = r"\Sigma^{("+ basis_letter +r")}"

            if split_path[-1] == "add_distmat_aug":
                name = r"D^{+"+ aug_basis +r"}"
            # multiplicative
            elif split_path[-1] == "mul_distmat_aug":
                name = r"D^{\odot "+ aug_basis +r"}"
            # sigma as fv
            elif split_path[-1] == "sigma_as_fv":
                name = r"D^{"+ aug_basis +r"}"
            else:

                if split_path[4] == "fv_weighting_nomagic":
                    if split_path[3] == "True":
                        c_term = r"f \left(\sqrt{\Sigma^{(" + basis_letter + r")}} \right)"
                    else:
                        c_term = r"f \left(\Sigma^{(" + basis_letter + r")} \right)"
                elif split_path[6] == "eps":
                    c_term = r"c_{opt}"
                else:
                    aux_letter = {
                        "model": "M",
                    ...
                    #lambda
                    if split_path[8] == "True":
                        aug_aux = r"\sqrt{\Sigma^{("+ aux_letter +r")}}"
                    else:
                        aug_aux = r"\Sigma^{("+ aux_letter +r")}"

                    aux_norm = {
                        "1_l1": r"\left(\left\| "+ aug_aux +r" \right\|_1 \right)^{-1}",
                        "1_l1ln": r"\left(\left\| \ln{\left("+ aug_aux +r"\right)} \right\|_1 \right)^{-1}",
                        "l2": r"\left\| "+ aug_aux +r" \right\|_2",
                        "entropy": r"\mathcal{H}\left( "+ aug_aux +r" \right)"
                    }[split_path[9]]

                    c_term = r"\lambda_{opt} \cdot " + aux_norm
                


                name = r"D^{\mu," + aug_basis + r"}, c="+c_term
        #print(r"$"+ name + r"$ & $"+ mAP_tex + r" \pm " + mAP_std_tex + r"$ & $" + r1_tex + r" \pm " + r1_std_tex + r"$ &~"+ str(i) +r"~\\")
        #print(r"$"+ name + r"$ & $"+ mAP_tex + r" \pm " + mAP_std_tex + r"$ & $" + r1_tex + r" \pm " + r1_std_tex + r"$ \\")
    exit()
    print("") 
...
tabulate_all_results_summary(paths=[
    "UAL_lambda_A_data.json", 
    "UAL_lambda_A_dist.json", 
    "UAL_lambda_A_model.json", 
    "UAL_lambda_B_data.json", 
    "UAL_lambda_B_dist.json", 
    "UAL_lambda_B_dist.json1", 
    "UAL_lambda_B_model.json", 
    "UAL_lambda_MoDi.json", 
    "UAL_lambda_MoMo.json",
    "UAL_results_model.json",
    "UAL_results_data.json",
    "UAL_results_dist.json"
    ], precision=5, exclude_keys=["best_avg_res-mAP"], whitelist_paths_with_substr=[])
# tabulate_all_results_summary(path="all_UAL_results_torch.json", precision=4, exclude_keys=["best_avg_res-mAP"], whitelist_paths_with_substr=["vanilla"])
...
"""get_all_performances(
  ...
  # bsub -w "done(2643946)" -gpu "num=1" -J "data_B" -q "BatchGPU" -outdir . -o ./lout.%J -e ./lerr.%J python ../../misc/data_stats.py 
