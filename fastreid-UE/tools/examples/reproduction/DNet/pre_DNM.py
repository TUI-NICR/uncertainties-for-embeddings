...
#SBATCH --output=/results_nas/ange8547/DNet/fastreid/pre_DNM/%a/slurm-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=rDNet_pre_DNM
...
def volatile_config(cfg):
    ...
    if slurm_array_id != None:
        ...
        OUTPUT_ADDENUM = os.path.join("pre_DNM", str(slurm_array_id))

        #PRETRAIN_ADDENUM = os.path.join("random", str(slurm_array_id), "model_best.pth")
