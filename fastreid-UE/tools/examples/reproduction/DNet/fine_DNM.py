...
#SBATCH --output=/results_nas/ange8547/DNet/fastreid/fine_DNM/%a/slurm-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=rDNet_fine_DNM

# make this into an array of ten jobs
#SBATCH --array=0-9
...
def volatile_config(cfg):
    ...
    if slurm_array_id != None:
        ...
        OUTPUT_ADDENUM = os.path.join("fine_DNM", str(slurm_array_id))

        PRETRAIN_ADDENUM = os.path.join("pre_DNM", str(slurm_array_id), "model_final.pth") # they use model_final
        ...
        cfg.DATASETS.TESTS = ("MarketDNet",)
        cfg.DATASETS.NAMES = ("MarketDNet",)
# -------------------------------------------------------------
###############################################################
