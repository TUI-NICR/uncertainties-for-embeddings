...
#SBATCH --output=/results_nas/ange8547/DNet/fastreid/fine_Market/%a/slurm-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=rDNet_fine_Market
...
def volatile_config(cfg):
    ...
    if slurm_array_id != None:
        ...
        OUTPUT_ADDENUM = os.path.join("fine_Market", str(slurm_array_id))

        PRETRAIN_ADDENUM = os.path.join("pre_Market", str(slurm_array_id), "model_final.pth") # they use model_final
