...
#SBATCH --output=/results_nas/ange8547/mod_arch/UAL/experiments/Neckless/%a/slurm-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=UAL_Neckless
...
def change_out_dir(cfg):
    ...
    cfg.MODEL.NECK.NAME = "Identity"

    slurm_array_id = get_array_id()
    if slurm_array_id != None:

        OUTPUT_ADDENUM = os.path.join("experiments","Neckless", slurm_array_id)
