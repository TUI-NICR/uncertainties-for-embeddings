...
#SBATCH --output=/results_nas/ange8547/mod_arch/UAL/experiments/Dropout_T=1/%a/slurm-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=UAL_Dropout_T=1
...
###############################################################
# -------------------------------------------------------------
RUN_TYPE = "UAL_Dropout_T=1"
WANDB_PROJECT = "mod_arch-UAL_experiments"

SEED_LIST = [37660560, 3370138, 40738399, 45854103, 12019182, 43853336, 24708352, 23367063, 48796365, 24597088]

def change_out_dir(cfg):

    cfg.MODEL.META_ARCHITECTURE = "Uncertainty"

    slurm_array_id = get_array_id()
    if slurm_array_id != None:

        OUTPUT_ADDENUM = os.path.join("experiments", "Dropout_T=1", slurm_array_id)
        ...
        cfg.SEED = SEED_LIST[int(slurm_array_id)]
# -------------------------------------------------------------
###############################################################
