...
#SBATCH --output=/results_nas/ange8547/mod_arch/UAL/experiments/Gaussian_3D/%a/slurm-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=UAL_Gaussian_3D
...
def change_out_dir(cfg):
    ...
    cfg.MODEL.NECK.BAYESIAN_LAYER = "Gaussian"

    slurm_array_id = get_array_id()
    if slurm_array_id != None:

        OUTPUT_ADDENUM = os.path.join("Gaussian_3D", slurm_array_id)
