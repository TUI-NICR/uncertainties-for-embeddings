...
#SBATCH --output=/results_nas/ange8547/mod_arch/bagtricks/center/%a/slurm-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=pretrain_baseline_bagtricks__center
...
###############################################################
# -------------------------------------------------------------
RUN_TYPE = "pre-bagtricks-center"
WANDB_PROJECT = "mod_arch"

def configure_volatile(cfg):

    cfg.MODEL.LOSSES.NAME += ("CenterLoss",)

    slurm_array_id = get_array_id()
    if slurm_array_id != None:

        ADDENUM = os.path.join("center", str(slurm_array_id))
