...
#SBATCH --output=/results_nas/ange8547/mod_arch/PFE/experiments/LFU/%a/slurm-%a-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=PFE_LFU
...
#RUN_TYPE = "PFE_"
WANDB_PROJECT = "mod_arch-PFE_experiments"

def change_out_dir(cfg):

    cfg.MODEL.LOSSES.NAME += ("FeatureUncertaintyLoss",)
    cfg.MODEL.LOSSES.FU.SCALE = 1.0

    slurm_array_id = get_array_id()
    if slurm_array_id != None:

        pretrain_id = (int(slurm_array_id))

        # set identifying config options
        cfg.run_type = "PFE_LFU_1_smallscale" # TODO: need to adjust this in pfe var head

        OUTPUT_ADDENUM = os.path.join("experiments", "LFU", str(pretrain_id))

        PRETRAIN_ADDENUM = os.path.join("naive", str(pretrain_id), "model_best.pth")
