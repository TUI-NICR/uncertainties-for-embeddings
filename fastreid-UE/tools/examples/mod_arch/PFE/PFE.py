...
#SBATCH --output=/results_nas/ange8547/mod_arch/PFE/fixed+BN/%a/slurm-%j.out
...
###############################################################
# -------------------------------------------------------------
RUN_TYPE = "PFE_fixed+BN"
WANDB_PROJECT = "PFE_fixed"

def change_out_dir(cfg):

    #cfg.MODEL.LOSSES.MLS.SCALE = -1.0

    slurm_array_id = get_array_id()
    if slurm_array_id != None:

        OUTPUT_ADDENUM = os.path.join("fixed+BN", str(slurm_array_id))
