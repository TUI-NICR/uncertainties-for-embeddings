...
#SBATCH --output=/results_nas/ange8547/mod_arch/DNet/bb_random/random/%a/slurm-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=DNet_bb-random_random
...
###############################################################
# -------------------------------------------------------------
RUN_TYPE = "dnet_bb-random_random"
...
def change_out_dir(cfg):
    ...
    if slurm_array_id != None:
        ...
        OUTPUT_ADDENUM = os.path.join("bb_random", "random", str(slurm_array_id))
