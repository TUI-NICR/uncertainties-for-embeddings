...
#SBATCH --output=/results_nas/ange8547/mod_arch/DNet/test/slurm-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=DNet_test
...
from fastreid.engine import UncertaintyTrainer, default_argument_parser, default_setup, launch, DefaultTrainer
...
def change_out_dir(cfg):
    ...
    cfg.SOLVER.MAX_EPOCH = 6

    
    OUTPUT_ADDENUM = os.path.join("test")

    PRETRAIN_ADDENUM = os.path.join("naive", str(0), "model_best.pth")
    ...
    #cfg.MODEL.LOSSES.NAME = ("CrossEntropyLoss", "FeatureUncertaintyLoss")
    cfg.MODEL.LOSSES.NAME += ("TripletLoss",)
    ...
    #cfg.MODEL.CHECKPOINT_REMAP = [('heads.weight','crown.logit_layer.weight')]
# -------------------------------------------------------------
###############################################################
...
def main(args):
    ...
    #summary(model, input_size=(64, 3, 256, 128))
    ...
    trainer = UncertaintyTrainer(cfg) # I could also use the default trainer for training and the other one only for eval but I don't think that would actually be simpler
    #trainer = DefaultTrainer(cfg)

    trainer.resume_or_load(resume=args.resume) # default: False (when omitted)
