...
#SBATCH --output=/results_nas/ange8547/mod_arch/DNet/experiments/slurm_4/slurm-%x-%a-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=DNet_BoT_experiment4
...
WANDB_PROJECT = "mod_arch-DNet_experiment4"
...
def volatile_config(cfg):
    ...
    if slurm_array_id != None:
        ...
        cfg.MODEL.LOSSES.FU.USE_PAPER_FORMULA = True

        if int(slurm_array_id) % 2 == 0:
            # even
            experiment_type = "plain-pFU"

        else:
            # odd
            experiment_type = "RPCT-pFU"
            setup_rea(cfg)
            setup_padding(cfg)
            setup_triplet(cfg)
            setup_center(cfg)
