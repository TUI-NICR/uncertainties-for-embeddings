...
#SBATCH --output=/results_nas/ange8547/mod_arch/DNet/experiments/b64_3/%a/slurm-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=DNet_b64_3
...
###############################################################
# -------------------------------------------------------------
RUN_TYPE = "dnet_b=64_3"
WANDB_PROJECT = "mod_arch"

def change_out_dir(cfg):

    cfg.SOLVER.IMS_PER_BATCH = 64
    cfg.SOLVER.BASE_LR = 0.0002 # double b, quadruple eta

    slurm_array_id = get_array_id()
    if slurm_array_id != None:

        OUTPUT_ADDENUM = os.path.join("experiments", "b64_3", str(slurm_array_id))
