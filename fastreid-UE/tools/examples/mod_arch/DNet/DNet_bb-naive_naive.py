...
#SBATCH --output=/results_nas/ange8547/mod_arch/DNet/bb_naive/naive/%a/slurm-%j.out

# specify job name that is seen in slurm info
#SBATCH --job-name=DNet_bb-naive_naive
...
def change_out_dir(cfg):
    ...
    # cfg.OUTPUT_DIR = "/results_nas/ange8547/mod_arch/DNet/experiments/RPCT/4" 
    # cfg.MODEL.WEIGHTS = "/results_nas/ange8547/mod_arch/DNet/experiments/RPCT/4/model_best.pth"
    if slurm_array_id != None:

        OUTPUT_ADDENUM = os.path.join("bb_naive", "naive", str(slurm_array_id))

        PRETRAIN_ADDENUM = os.path.join("naive", str(slurm_array_id), "model_best.pth")
