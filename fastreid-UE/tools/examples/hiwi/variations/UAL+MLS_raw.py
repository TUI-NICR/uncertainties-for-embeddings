...
MLS_WEIGHTS = [0.0000001, 0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0, 100.0]
...
def volatile_config(cfg):
    ...
    #cfg.OUTPUT_DIR = "/usr/scratch4/angel8547/results/variations/UAL+MLS"

    cfg.MODEL.LOSSES.NAME += ("MutualLikelihoodScoreLoss",)
    cfg.MODEL.LOSSES.MLS.USE_RAW_MEAN = True
    ...
    #cfg.TEST.EVAL_ENABLED_IN = "10,20,30,40,45-55,60,70,75,80,84,88,92,94,96,98,100-120"
    #cfg.MODEL.DYNAMIC_CONFIG = [[2, "loss_kwargs.mls.scale", 1337]]
    ...
    if not is_test_run:
        cfg.MODEL.LOSSES.MLS.SCALE = MLS_WEIGHTS[int(job_array_id)-10] # starting at ten
    else:
        cfg.MODEL.LOSSES.MLS.SCALE = 0.001


    if job_array_id != None and job_array_id != "test":
        try:
            os.mkdir(os.path.join(cfg.OUTPUT_DIR, str(MLS_WEIGHTS[int(job_array_id)-10])))
        except Exception:
            pass
