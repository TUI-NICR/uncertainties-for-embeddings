...
def default_setup(cfg, args):
    ...
    # check for proper argument type and extract seed for this process if applicable
    if cfg.SEED == None:
        seed = cfg.SEED
    elif type(cfg.SEED) == int:
        seed = cfg.SEED
    elif type(cfg.SEED) == list:
        # allow for entering of tuples (rank, seed)
        seed_provided_for_current_process = False
        for given_seed_tuple in cfg.SEED:
            assert len(given_seed_tuple) == 2, f"If you provide a list of seeds, it must be in the format [(rank1, seed1), (rank2, seed2), ...]. This error was caused by {given_seed_tuple}."
            if rank == int(given_seed_tuple[0]):
                seed_provided_for_current_process = True
                seed_for_current_process = int(given_seed_tuple[1])
                break
        
        if seed_provided_for_current_process:
            seed = seed_for_current_process
        else:
            raise ValueError(f"Process-specific definition of seeds was used but no seed was provided for process of rank {rank}. Adjust the cfg.SEED option.")
        
    else:
        raise TypeError(f"cfg.SEED must be of type None, list(tuple(2)), or int but got {type(cfg.SEED)}.")
    
    # set seed
    seed = seed_all_rng(seed=seed) # default: None -> generate randomly

    # save used seeds to config for proper logging
    is_frozen = cfg.is_frozen() # maintain freeze status of cfg
    if is_frozen:
        cfg.defrost()
    cfg.SEEDS_USED.append((rank, seed))
    if is_frozen:
        cfg.freeze()

    if cfg.TEST.EVAL_ENABLED_IN != None:
        # parse string and extend to full list
        full_list = []
        for part in ''.join(cfg.TEST.EVAL_ENABLED_IN.split()).split(','): 
            if '-' in part:
                start, end = map(int, part.split('-'))
                full_list.extend(range(start, end + 1))
            elif part.isdigit():
                full_list.append(int(part))
            else:
                raise ValueError("EVAL_ENABLED_IN should be None or a string like \"1,10,12-14,16,20-25\"")
    else:
        full_list = list(range(cfg.SOLVER.MAX_EPOCH + 1))
    is_frozen = cfg.is_frozen() # maintain freeze status of cfg
    if is_frozen:
        cfg.defrost()
    cfg.TEST.EVAL_ENABLED_IN = full_list
    if is_frozen:
        cfg.freeze()
...
class DefaultTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        self.checkpointer = Checkpointer(
            ...
            checkpoint_remap=cfg.MODEL.CHECKPOINT_REMAP,
            optimizer=optimizer,
            **self.scheduler,
        )
    ...
    def build_hooks(self):
        ...
        if cfg.MODEL.DYNAMIC_CONFIG != None:
            ret.append(hooks.DynamicModelConfig(
                self.model,
                cfg.MODEL.DYNAMIC_CONFIG
            ))
        ...
        def test_and_save_results(last_eval=False):
            if last_eval and not self.cfg.TEST.EXTRA_END_EVAL:
                last_eval = None
            self._last_eval_results = self.test(self.cfg, self.model, last_eval=last_eval)
            if wandb.run is not None: # only log if initialized
                wandb.log(step=self.iter, data={'eval': dict(self._last_eval_results)})
            return self._last_eval_results

        # Do evaluation before checkpointer, because then if it fails,
        # we can use the saved checkpoint to debug.
        ret.append(hooks.EvalHook(cfg.TEST.EVAL_PERIOD, test_and_save_results, extra_end_eval=self.cfg.TEST.EXTRA_END_EVAL, allowed_epochs=cfg.TEST.EVAL_ENABLED_IN))

        if comm.is_main_process():
            self._periodic_checkpointer_hook = hooks.PeriodicCheckpointer(self.checkpointer, cfg.SOLVER.CHECKPOINT_PERIOD, keep_only_best_checkpoint=cfg.KEEP_ONLY_BEST_CHECKPOINT, allowed_epochs=cfg.TEST.EVAL_ENABLED_IN)
            ret.append(self._periodic_checkpointer_hook)
    ...
    @classmethod
    def test(cls, cfg, model, last_eval=False):
        """
        Args:
            cfg (CfgNode):
            model (nn.Module):
