...
from fastreid.utils.BA_UE_utils import setattr_rec
...
class EvalHook(HookBase):
    ...
    def __init__(self, eval_period, eval_function, extra_end_eval=False, allowed_epochs=None):
        """
        Args:
        ...
        self.extra_end_eval = extra_end_eval
        self.allowed_epochs = allowed_epochs

    def _do_eval(self, last_eval=False):
        results = self._func(last_eval)
    ...
    def after_epoch(self):
        next_epoch = self.trainer.epoch + 1 # TODO: why does this use next_epoch for eval period checks?
        if self._period > 0 and next_epoch % self._period == 0 and self.trainer.epoch in self.allowed_epochs:
            self._do_eval()

    def after_train(self):
        next_epoch = self.trainer.epoch + 1
        # This condition is to prevent the eval from running after a failed training
        if (next_epoch % self._period != 0 or self.extra_end_eval) and next_epoch >= self.trainer.max_epoch:
            self._do_eval(last_eval=True)
...
class DynamicModelConfig(HookBase):
    def __init__(self, model, config) -> None:
        super().__init__()
        if isinstance(model, DistributedDataParallel):
            model = model.module
        self.model = model
        self.config = config # [[epoch, key, value], ...]

    def before_epoch(self):
        for index, option in enumerate(self.config):
            if self.trainer.epoch == option[0]:
                self.config.pop(index)
                setattr_rec(self.model, option[1], option[2])
