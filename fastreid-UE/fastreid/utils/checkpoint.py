...
class Checkpointer(object):
    ...
    def __init__(
        ...
        self.checkpoint_remap = checkpoint_remap
    ...
    def load(self, path: str, checkpointables: Optional[List[str]] = None) -> object:
        ...
        checkpoint = self._remap_checkpoint_keys(checkpoint)
    ...
    def _remap_checkpoint_keys(self, checkpoint):
        
        for index, instruction in enumerate(self.checkpoint_remap):

            # move to new position
            oldKey = instruction[0]
            newKey = instruction[1]

            if oldKey[-1] == "*":
                # find all keys that match the before the asterisk
                matching_keys = [key for key in checkpoint["model"] if key.startswith(oldKey[:-1])]
                # add instructions to the list where the star in newKey
                new_instruction_counter = 0
                for matched_key in matching_keys:
                    new_instruction = [matched_key, newKey[:-1] + matched_key[len(oldKey[:-1]):]]
                    if len(instruction) == 3:
                        new_instruction.append(instruction[2])
                    #print(new_instruction)
                    self.checkpoint_remap.insert(index+1+new_instruction_counter, new_instruction)
                    new_instruction_counter += 1
                # is replaced by the gained values (after the asterisk)
                continue

            if oldKey != newKey:
                checkpoint["model"][newKey] = checkpoint["model"][oldKey]
                del checkpoint["model"][oldKey]

            # add or remove singleton dimensions
            if len(instruction) == 3: 
                num_unsqueeze = instruction[2]
                if num_unsqueeze > 0:
                    for i in range(num_unsqueeze):
                        checkpoint["model"][newKey].unsqueeze_(-1)
                elif num_unsqueeze < 0:
                    for i in range(num_unsqueeze):
                        checkpoint["model"][newKey].squeeze_(-1)
                else:
                    checkpoint["model"][newKey].squeeze_()
        return checkpoint
...
class PeriodicCheckpointer:
    ...
    def __init__(self, checkpointer: Any, period: int, max_epoch: int = None, keep_only_best_checkpoint=False, allowed_epochs=None):
        """
        Args:
    ...
    def step(self, epoch: int, **kwargs: Any):
        ...
        """
        epoch = int(epoch)
        if epoch in self.allowed_epochs:
            ...
            if (epoch + 1) % self.period == 0 and epoch < self.max_epoch - 1: 
                ...
                # only save model_best when this config option is set
            ...
            if epoch >= self.max_epoch - 1: # end of training
                if additional_state["metric"] > self.best_metric:
                    self.checkpointer.save(
                        "model_best", **additional_state
                    )
