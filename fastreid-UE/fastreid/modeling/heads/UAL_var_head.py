...
from fastreid.utils.BA_UE_utils import safe_log1p_exp
...
class UAL_var_head(nn.Module):
    """
    The var head from UAL.
    """
    ...
    def forward(self, features):
        ...
        var = safe_log1p_exp(self.layer(features))
        #var = torch.log1p(torch.exp(self.layer(features)))

        var_vector = self.pool(var)[..., 0, 0]

        return {
            "variance": var,
            "variance_vector": var_vector
        } 