from fastreid.layers.batch_norm import BatchNorm2_5d
from fastreid.layers.UAL import BayesDropoutConv2_5d
...
"""
The only changes from this to the original UAL head are such that it works with UncertaintyReidEvaluator.
"""
...
class BayesHead(nn.Module):
    ...
    def __init__(
        ...
        self.pool_layer = pooling.GlobalAvgPool2_5d()

        self.neck_feat = neck_feat
        self.feat_dim = feat_dim
        self.test_time_ens = test_time_ens
        self.ual_layer = ual_layer
        ...
        if with_bnneck:
            ...
            neck.append(get_norm(norm_type, feat_dim, bias_freeze=True, enable_3d=True))
        ...
        self.logit_layer = nn.Conv3d(self.feat_dim, num_classes, 1, bias=False)
    ...
    def _make_bayes(self):
        ...
        """ --------------------------------------------------------------------------- BA_UE
        self has no attribute `p`. Since p is optional in Bayes_head, we try to omit it here and hope that this is the same as the authors did.
        In the paper it says that they epirically set p to 0.7 so this should be correct.
        """
        self.bayes_head = Bayes_head(in_dim=in_dim, mid_dim=[bayes_dim0, bayes_dim1], out_dim=in_dim, ual_layer=self.ual_layer) 
    ...
    def reset_parameters(self) -> None:
        ...
        nn.init.normal_(self.logit_layer.weight, std=0.01)
    ...
    def _reset_weight(self): # called during test
        return
        self.bayes_models = [self.bayes_head]
        self.test_state = [] # = [[bayes_head.state_dict], [...], ...]
        for i in range(self.test_time_ens):
            tmp = []
            for m in self.bayes_models: # one iteration
                m.reset_weight()
    ...
    def forward(self, features, targets=None):
        ...
        if self.training:
            ...
            g_var = feat_var.mean(dim=-1).mean(dim=-1)
            ...
            feat_mean_raw = []
            for i in range(self.test_time_ens):
                self.load_bayes_model(i) # unnecessary. it just samples new weights on each call anyway (if it didn't have the train/test weights difference)
                tmp_features = self.bayes_head(features)
                feat_mean = self.mean_head(tmp_features)
                feat_mean_raw.append(self.pool_layer(feat_mean)[..., 0, 0].unsqueeze(0))
                tmp_feat_var = torch.log1p(torch.exp(self.var_head(tmp_features)))
                tmp_features = feat_mean / tmp_feat_var

                tmp_pool_feat = self.pool_layer(tmp_features) # (B,C,1,1)
                tmp_neck_feat = self.bottleneck(tmp_pool_feat)
                tmp_neck_feat = tmp_neck_feat[..., 0, 0] # (B,C)
                pool_feat.append(tmp_pool_feat.unsqueeze(dim=0))
                neck_feat.append(tmp_neck_feat.unsqueeze(dim=0))
                feat_var.append(tmp_feat_var.mean(dim=-1).mean(dim=-1).unsqueeze(dim=0))
            pool_feat = torch.cat(pool_feat, dim=0) # (B, C, 1, 1)
            mean_vectors = torch.cat(neck_feat, dim=0) # (B, C) # this is eval, so we return normalized
            variance_vectors = torch.cat(feat_var, dim=0)
            feat_mean_raw = torch.cat(feat_mean_raw, dim=0)

            return {
                "mean_vector": mean_vectors.mean(dim=0).squeeze(dim=2),
                "variance_vector": variance_vectors.mean(dim=0).squeeze(dim=2),
                #"variance_of_mean_vector": mean_vectors.var(dim=0),
                "variance_of_mean_vector": feat_mean_raw.var(dim=0).squeeze(dim=2),
                "variance_of_variance_vector": variance_vectors.var(dim=0).squeeze(dim=2)
            }

        # Evaluation
        # fmt: off
        #if not self.training: return neck_feat # TODO: mod_arch UAL return value
        # fmt: on

        #neck_feat = neck_feat.squeeze(2)
        #pool_feat = pool_feat.squeeze(2)
        ...
        logits = self.logit_layer(neck_feat)

        logits= logits.squeeze(dim=2)[..., 0, 0]
        neck_feat = neck_feat.squeeze(dim=2)
        pool_feat = pool_feat.squeeze(dim=2)

        # Pass logits.clone() into cls_layer, because there is in-place operations
        #cls_outputs = self.cls_layer(logits.clone(), targets)
        cls_outputs = logits.clone()
...
class Bayes_Dropout_Conv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.prob = prob
        self.distribution = torch.distributions.Bernoulli(prob)
    ...
    def sample_weight(self, nums):
        ...
        for i in range(nums):
            ...
            weight = self.weight * self.distribution.sample(self.weight.shape).to(self.weight.device)
            #weight *= 0.7 / self.prob
            if self.bias is not None:
                #bias += self.bias * bernolli.sample(self.bias.shape).to(self.bias.device)
                bias = self.bias * self.distribution.sample(self.bias.shape).to(self.bias.device)
                #bias *= 0.7 / self.prob
            else:
                bias = None
        #weight /= nums
    ...
    def forward(self, input):
        if self.training or True:
            ...
            weight, bias = self.weight_test, self.bias_test
        # this is equivalent to doing nums convolutions and then taking the average
        return F.conv2d(input, weight, bias, self.stride, self.padding, self.dilation, self.groups)
...
class Bayes_head(nn.Module):
    def __init__(self, in_dim=1920, mid_dim=[1024, 512], out_dim=1024, p=0.7, ual_layer='Dropout'):
        ...
        self.p = p
        # self.p = 0.95
        #self.layer.add_module(name=f"unsqueeze", module=UnsqueezeLayer(2))
        for i in range(self.bayes_count):
            in_dim, out_dim = dim_list[i], dim_list[i + 1]
            self.bayes_index.append(i * 3)
            if ual_layer == "Gaussian":
                 self.layer.add_module('Bayes_{}'.format(i),
                                       Bayes_Gaussion_Conv2d(in_channels=in_dim, out_channels=out_dim, kernel_size=1,
                                                             stride=1, padding=0))
            elif ual_layer == "Dropout":
                """self.layer.add_module('Bayes_{}'.format(i),
                                    Bayes_Dropout_Conv2d(in_channels=in_dim, out_channels=out_dim, kernel_size=1,
                                                        stride=1, padding=0, prob=self.p))"""
                #self.layer.add_module(name=f"unsqueeze_{i}", module=UnsqueezeLayer(2))
                self.layer.add_module('Bayes_{}'.format(i),
                                    BayesDropoutConv2_5d(in_channels=in_dim, out_channels=out_dim, kernel_size=1,
                                                        stride=1, padding=0, prob=self.p))
                #self.layer.add_module(name=f"squeeze_{i}", module=SqueezeLayer(2))
            else:
                raise ValueError(f"{ual_layer} is not a valid ual_layer identifier.")
            #self.layer.add_module('BN_{}'.format(i), nn.BatchNorm2d(out_dim, momentum=BN_MOMENTUM))
            self.layer.add_module('BN_{}'.format(i), BatchNorm2_5d(out_dim))
            if i < self.bayes_count - 1:
                self.layer.add_module('ReLU_{}'.format(i), nn.ReLU(inplace=True))
        #self.layer.add_module(name=f"squeeze", module=SqueezeLayer(2))
...
class FCN_head(nn.Module):
    def __init__(self, in_dim=1920, mid_dim=[256], out_dim=1024):
        ...
        for i in range(self.bayes_count):
            ...
            #self.layer.add_module('BN_{}'.format(i), nn.BatchNorm2d(out_dim, momentum=BN_MOMENTUM))
            self.layer.add_module('BN_{}'.format(i), BatchNorm2_5d(out_dim))#, momentum=BN_MOMENTUM))
            if i < self.bayes_count - 1:
                self.layer.add_module('ReLU_{}'.format(i), nn.ReLU(inplace=True))
        #self.layer.add_module(name=f"squeeze", module=SqueezeLayer(2))
