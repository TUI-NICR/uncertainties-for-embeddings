...
def build_resnet_backbone(cfg):
    ...
    if pretrain:
        ...
        # actually, checkpoint loading is done using MODEL.WEIGHTS, not MODEL.BACKBONE.PRETRAIN_PATH
        # the authors of fastreid seem to have forgotten that they changed the checkpoint format
        """if "model" in state_dict:
            new_state_dict = {}
            for key, value in state_dict["model"]:
                if key.split(".")[0] == "backbone":
                    # since we are currently loading the resnet backbone, we only need the backbone weights
