...
def build_transforms(cfg, is_train=True):
    ...
    if is_train:
        ...
        interpolation_mode = cfg.INPUT.INTERPOLATION_MODE
        ...
        if size_train[0] > 0:
            res.append(T.Resize(size_train[0] if len(size_train) == 1 else size_train, interpolation=interpolation_mode))
        ...
        if size_test[0] > 0:
            res.append(T.Resize(size_test[0] if len(size_test) == 1 else size_test, interpolation=interpolation_mode))
        if do_crop:
