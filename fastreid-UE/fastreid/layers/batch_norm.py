...
class BatchNorm2_5d(nn.Module):
    """
    Expects 5D input tensor (B,C,D,H,W). Will apply a 2D BN to each slice along the D axis and return a 5D Tensor again.
    """
    def __init__(self, num_features, eps=1e-05, momentum=0.1, weight_freeze=False, bias_freeze=False, weight_init=1.0,
                 bias_init=0.0, **kwargs):
        super().__init__()
        self.BN = BatchNorm(num_features, eps=eps, momentum=momentum, weight_freeze=weight_freeze, bias_freeze=bias_freeze, weight_init=weight_init,
                 bias_init=bias_init, **kwargs)
        
    def forward(self, input):
        # for every slice along D axis, perform 2D BN
        return torch.stack([self.BN(d_slice) for d_slice in torch.unbind(input, dim=2)], dim=2) 
...
def get_norm(norm, out_channels, enable_3d=False, parallel_BN_full_3d=False, **kwargs):
    """
    Args:
